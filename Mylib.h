...
int  thanh_phan_lien_thong_manh_find(cgraph_t g,CGRAPH_INTEGER find,cgraph_ivec_t* tpltm){
	...
	cgraph_ivec_t order_reverse     = cgraph_ivec_create(),
	              order_out_reverse = cgraph_ivec_create(),
	              father_reverse    = cgraph_ivec_create(),
	              dist_reverse      = cgraph_ivec_create();

	cgraph_dfs(r, 0, CGRAPH_OUT, true, &order_reverse, &order_out_reverse, &father_reverse, &dist_reverse); // dfs do thi nguoc

	bool *visited = calloc(g->n,sizeof(bool));

	//chay theo thu tu post giam dan cua dfs
	for(count=0,i=cgraph_ivec_size(order_out_reverse)-1;i>=0;--i){
		if(!visited[order_out_reverse[i]]){
			...
			cgraph_dfs(g, order_out_reverse[i], CGRAPH_OUT, false, &order, &order_out, &father, &dist); // dfs do thi xuoi

			if(find>=0&&find<g->n){
				for(j=0;j<g->n&&order[j]!=CGRAPH_NAN;++j){

					if(order[j]==find){					//Tim thay
						for(j=0;j<g->n&& order[j]!=CGRAPH_NAN;++j){
							if(!visited[order[j]]) cgraph_ivec_push_back(tpltm,order[j]);
						}
			...
			for(j=0;j<g->n&&order[j]!=CGRAPH_NAN;++j){				//Danh dau da duyet qua
				if(!visited[order[j]]) visited[order[j]]=true;
			}
	...
	cgraph_ivec_free(&order_reverse);
	cgraph_ivec_free(&order_out_reverse);
	cgraph_ivec_free(&father_reverse);
	cgraph_ivec_free(&dist_reverse);
	free(visited);
...
int thanh_phan_lien_thong_find(cgraph_t g,CGRAPH_INTEGER find,cgraph_ivec_t* tplt){
	...
	cgraph_ivec_t order     = cgraph_ivec_create(),
	              ...
	              father    = cgraph_ivec_create(),
	              dist      = cgraph_ivec_create();
	cgraph_dfs(g, find, CGRAPH_ALL, false, &order, &order_out, &father, &dist); //Khong duyet cac dinh ma dinh xuat phat khong di den duoc
...
int cgraph_kruskal(cgraph_t g,cgraph_rvec_t weights,cgraph_ivec_t *kruskal){
	int i,count;
	bool correct;
	...
	for(i=0,count=0;i<cgraph_ivec_size(ws);++i){
		...
		if(correct){
			if(kruskal!=NULL){
				cgraph_ivec_push_back(kruskal,ws[i]);
			}
		}
	}

	//xet co ton tai cay bao trum nho nhat
	correct=true;
...
int cgraph_prim(cgraph_t g,cgraph_rvec_t weights ,cgraph_ivec_t *prim){
			...
			for(j=0;j<neisize;++j){
				...
				if(!visited[tto]){
					...
					if(min<0||dists[tto]<dists[min]){
						// chua co dinh nao chua canh la min hoac canh chua dinh nay co trong so nho hon canh chua dinh min truoc do
						min=tto;
					}
				}
			}
	...
	for(i=1;i<g->n;++i){
		if(prim!=NULL){
			cgraph_ivec_push_back(prim,parents[i]);
		}
	}
	free(visited);
